...
def read_buckets(path):
    """Section-parse one rulebook into its raw buckets."""
...
def find_project_banned(target):
    """The nearest project ban list above the copy being checked, if any.

    A global skill still answers to the repo it is run in. A word this project
    cannot do without -- a genre term the audience searches for, a product name
    that collides with a banned word -- is a decision belonging to that repo,
    and it is recorded there rather than in the house list.
    """
    for parent in Path(target).resolve().parents:
        candidate = parent / PROJECT_BANNED
        if candidate.is_file() and candidate.resolve() != BANNED.resolve():
            return candidate
    return None


def parse_banned(target):
    """House lists overlaid with the project's own, into (words, warnings, structural, hedges)."""
    buckets = read_buckets(BANNED)
    project = find_project_banned(target)
    if project:
        for name, words in read_buckets(project).items():
            buckets[name] += words
    ...
    return (keep(buckets["word bans"]), keep(buckets["word warnings"]),
            buckets["structural bans"], buckets["hedges"]), project
...
def main():
    ...
    (word_bans, word_warns, structural, hedges), project_list = parse_banned(path)
    ...
    if project_list:
        print(f"  project ban list: {project_list}")
