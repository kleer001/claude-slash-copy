...
A word that is a tell in general copy can be the exact word one project's audience
searches for. That is what `## Exceptions` is for, and each one names the reason it is
load-bearing there.
...
Each record carries a rank, the signal, an example, where it was observed, and what
catches it. Work them in rank order: reader weight is not uniform, and a fabricated
...
The catalog grows because people keep finding new cracks. Append; do not edit the
method to accommodate a new entry.

1. **Require a real instance.** A tell needs copy that actually did it — a draft, a
   published page, a quoted example in a study or article. A shape you can imagine a
   model producing is not a tell yet. This is the rule the catalog lives or dies on:
   a hypothetical entry costs every future reader a real edit for no reason.
2. **Record provenance.** Short verbatim excerpt or concrete example, where it was
   observed (URL, publication, or `observed in own drafts`), and the capture date.
   Mark a constructed illustration `(pattern)` rather than passing it off as a quote.
3. **Append at the end** with the next free ID. IDs never move and never get reused.
4. **Say what catches it.** If a regex or a word list can, add the line to
   `banned.md` too and point the record at it. If only a human ear can, say
   `read aloud` and mean it.
5. **Supersede in place.** When a better-stated tell replaces an older one, mark the
   old record `superseded by TNNN` and leave it. What was once flagged and why it
   stopped being flagged is part of the record.
...
If the rewrite turned up a tell the catalog does not name, and you have the real
instance in front of you, append it to `tells.md` before you close out.

## Sources

Per-tell provenance lives in `tells.md`. These are the works the catalog and the
thresholds are built on.

- [Key Features to Distinguish Between Human- and AI-Generated Texts: Perspectives
  from University Professors](https://www.mdpi.com/3042-8130/2/1/2) — the ranked
  features carried as `professor-ranked` in `tells.md`.
